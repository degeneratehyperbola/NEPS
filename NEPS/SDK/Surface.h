...
public:
	...
	void drawCircle(T x, T y, int startRadius, int radius) noexcept
	{
		...
		for (int i = startRadius; i <= radius; ++i)
		{
			for (int j = 0; j < 12; ++j)
			{
				xs[j] = static_cast<int>(std::cos(Helpers::degreesToRadians(static_cast<float>(j * 30))) * i + x);
				ys[j] = static_cast<int>(std::sin(Helpers::degreesToRadians(static_cast<float>(j * 30))) * i + y);
			}
