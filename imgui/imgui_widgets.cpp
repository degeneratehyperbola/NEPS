...
bool ImGui::CloseButton(ImGuiID id, const ImVec2& pos)//, float size)
{
    ...
    if (hovered)
        window->DrawList->AddCircleFilled(center, ImMax(2.0f, g.FontSize * 0.5f + 1.0f), col, 16);
...
bool ImGui::CollapseButton(ImGuiID id, const ImVec2& pos)
{
    ...
    ImVec2 center = bb.GetCenter();
    if (hovered || held)
        window->DrawList->AddCircleFilled(center/*+ ImVec2(0.0f, -0.5f)*/, g.FontSize * 0.5f + 1.0f, bg_col, 16);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    } else if (*v)
    {
		...
		window->DrawList->AddRectFilled(check_bb.Min + ImVec2(pad, pad), check_bb.Max - ImVec2(pad, pad), check_col, fmaxf(style.FrameRounding - pad, 0.0f));
    }
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size)
{
	...
	// Drag & Drop mod for fucks sake
	// 3F to 4F and 4F to 3F or 4F everywhere
	// Fix for alpha alpha always remaining the same is in imguiCustom.cpp
    ...
    {
        if (flags & ImGuiColorEditFlags_NoAlpha)
            SetDragDropPayload(IMGUI_PAYLOAD_TYPE_COLOR_4F, &col_rgb, sizeof(float) * 3, ImGuiCond_Once);
        else
